...
function allLetter(uname) {
    ...
    if (uname_len == 0 || !uname.match(letters)) {
        document.getElementById("errorun").innerHTML = "Enter a valid name";
        return false;
...
function password_valid(pass) {
    ...
    if (password_len < 8 || !pass.match(pass_format)) {
        document.getElementById("error2").innerHTML = "Password must be minimum 8 characters long. Must have at least 1 number,1 lowercase and 1 uppercase letter.";
        return false;
    }
...
function address_valid(uadd) {
    ...
    if (address_len == 0) {
        document.getElementById("error3").innerHTML = "Address field is mendatory";;
        return false;
    }
    return true;
}
